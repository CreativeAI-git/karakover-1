...
function instrument_name($id)
{
    $CI = &get_instance();
    $CI->load->database();
    $CI->load->model('common');
    $instrument_detail = $CI->common->getData('tbl_instruments',array('id'=>$id),array('single'));

    if(!empty($instrument_detail)){
      return $instrument_detail['instrument'];
    }else{
     return false;
    }
}
