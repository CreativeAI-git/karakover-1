...
class Common extends CI_Model
{
    ...
    public function updateAdminData($table, $data, $where)
    {
    ...
    public function do_upload_img($file, $path)
    {
    ...
    {
        $config['upload_path']   = $path;
        $config['allowed_types'] = 'mp3|wav|m4a';
        $config['encrypt_name']  = true;
    ...
    function amazons3Upload($image_name, $fileTempName)
    {

        $awsAccessKey = 'AKIAZKVM3XCOJZCRVDB5';  //AWS account access key
        $awsSecretKey = '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';  //AWS account secret key
        $bucket_name  = 'glistener';  //Bucket name 
        $s3           = new S3($awsAccessKey, $awsSecretKey);
        $s3->putBucket($bucket_name);

        //move the file
        if ($s3->putObjectFile($fileTempName, $bucket_name, $image_name, S3::ACL_PUBLIC_READ_WRITE)) {
    ...
    public function amazons3UploadBucket($image_name, $fileTempName, $bucket_name)
    {
        ...
        $s3 = new Aws\S3\S3Client([
            ...
            'version' => 'latest',
            'credentials' => [
                // 'key'    => "AKIAZKVM3XCOCJUU5OHW",
                // 'secret' => "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",
    ...
    public function delete_songs_onS3Bucket($bucket, $keyname)
    {
        require 'vendor/autoload.php';
        $s3 = new Aws\S3\S3Client([
            'region'  => 'us-east-1',
